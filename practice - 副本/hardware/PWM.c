...
//	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);		//将JTAG引脚失能，作为普通GPIO引脚使用
	...
	TIM_TimeBaseInitStructure.TIM_Period = 72 - 1;					//计数周期，即ARR的值
...
void PWM_FAN(void) {
    static uint8_t direction = 0;  // 0: 递增, 1: 递减
    static uint16_t duty = 20;
    ...
    if (direction == 0) {
        ...
        duty -= 4;
        if (duty <= 20) direction = 0;
    }
    
    uint16_t ccr = (duty * 39) / 100;
    PWM_SetCompare1(ccr);
    }
...
void PWM_SetFanSpeed(float duty) {
    ...
    uint16_t ccr = (duty / 100.0f) * (TIM2->ARR + 1);
